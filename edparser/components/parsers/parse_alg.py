...
from edparser.components.parsers.chu_liu_edmonds import decode_mst
import numpy as np
...
def tarjan(parse_probs, length, tokens_to_keep, ensure_tree=True):
    ...
    length : NDArray
        sentence length including ROOT
...
def mst_then_greedy(arc_scores, rel_scores, mask, root_rel_idx, rel_idx=None):
    ...
    return tree, add_secondary_arcs(arc_scores, rel_scores, tree, root_rel_idx)
...
def add_secondary_arcs(arc_scores, rel_scores, tree, root_rel_idx, parse_preds=None):
    if not isinstance(tree, np.ndarray):
        tree = np.array(tree)
    if parse_preds is None:
...
def adjust_root_score_then_add_secondary_arcs(arc_scores, rel_scores, tree, root_rel_idx):
    ...
    rel_scores[:, :, root_rel_idx] = -float('inf')
    return add_secondary_arcs(arc_scores, rel_scores, tree, root_rel_idx, parse_preds)


def main():
    cdroot()
    # arc_scores, rel_scores, mask = load_pickle('arm.pkl')
    # i = 1
    # print(mst_then_greedy(arc_scores[i], rel_scores[i], mask[i]))
    # for i in range(len(mask)):
    #     mst_then_greedy(arc_scores[i], rel_scores[i], mask[i])
