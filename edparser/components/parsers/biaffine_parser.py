...
from edparser.components.parsers.parse_alg import unique_root, adjust_root_score, chu_liu_edmonds
from edparser.layers.transformers.loader import build_transformer

import edparser
from edparser.common.keras_component import KerasComponent
from edparser.components.parsers.alg_tf import tarjan, rel_argmax
from edparser.components.parsers.biaffine.model import BiaffineModel, StructuralAttentionModel
from edparser.transform.conll_tf import CoNLL_DEP_Transform, CoNLL_Transformer_Transform, CoNLL_SDP_Transform
from edparser.layers.embeddings.util_tf import build_embedding
from edparser.layers.transformers.tf_imports import PreTrainedTokenizer, TFAutoModel, TFPreTrainedModel, AutoTokenizer, \
    TFAutoModelWithLMHead, BertTokenizerFast, TFAlbertForMaskedLM, AlbertConfig, BertTokenizer, TFBertModel
from edparser.layers.transformers.utils import build_adamw_optimizer
from edparser.metrics.parsing.labeled_f1 import LabeledF1
from edparser.metrics.parsing.labeled_score import LabeledScore
from edparser.utils.io_util import get_resource
from edparser.utils.util import merge_locals_kwargs, DummyContext
...
class BiaffineTransformerDependencyParser(BiaffineDependencyParser, tf.keras.callbacks.Callback):
    ...
    def build_model(self, transformer, training, **kwargs) -> tf.keras.Model:
        ...
        model = BiaffineModel(self.config, transformer=transformer)
        return model
...
class BiaffineTransformerSemanticDependencyParser(BiaffineTransformerDependencyParser):
    ...
    def get_loss(self, arc_scores, rel_scores, arcs, rels, mask, arc_loss, rel_loss):
        return BiaffineSemanticDependencyParser.get_loss(self, arc_scores, rel_scores, arcs, rels, mask, arc_loss,
                                                         rel_loss)
    ...
    def decode(self, arc_scores, rel_scores, mask):
        return BiaffineSemanticDependencyParser.decode(self, arc_scores, rel_scores, mask)


class StructuralAttentionDependencyParser(BiaffineTransformerDependencyParser):
    ...
    def build_model(self, transformer, training, masked_lm_embed=None, **kwargs) -> tf.keras.Model:
        ...
        if masked_lm_embed:
            ...
            masked_lm_embed = tf.transpose(masked_lm_embed.embeddings)
